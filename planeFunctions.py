...
class FixedWingControl(Node):
    def __init__(self):
        ...
        self.spiral_index = None
        self.spiral_setpoints = None
        self.sprial_altitude = None
    ...
    def gps_callback(self, msg):
        if self.home_lat is None and self.home_lon is None and self.home_alt is None:
            self.home_lat = msg.latitude_deg
            self.home_lon = msg.longitude_deg
            self.home_alt = msg.altitude_msl_m
            self.get_logger().info(f"Set GPS home: {self.home_lat}, {self.home_lon}, {self.home_alt}")
    ...
    def full_takeoff(self, takeoff_altitude = None):
            ...
            if transition_to_offboard:
                ...
                self.FlightState = FlightState.SPIRAL
    ...
    def publish_waypoint_setpoint(self, x, y, altitude):
        ...
        msg = TrajectorySetpoint()
        self.vehicle_status.nav_state = VehicleStatus.NAVIGATION_STATE_ORBIT
        # Position setpoint
        msg.position = [
            float(x),  # X coordinate in local frame
            float(y),  # Y coordinate in local frame
            float(-altitude)  # Z coordinate (negative for altitude in NED frame)
        ]
...
#Matt extension
    def create_spiral_setpoint(self, w, dt, start_position, sprial_altitude):
        if not self.start_position:
            return
        ...
        scale = w / (2 * math.pi)
        self.spiral_setpoints = []
        self.sprial_altitude = sprial_altitude
 
        t = t_start
        while t <= t_end:
            x = scale * t * math.cos(t) + start_position[0]
            y = scale * t * math.sin(t) + start_position[1]
            self.spiral_setpoints.append((x, y))
            t += dt

        for x, y in self.spiral_setpoints:
            print(f"X: {x}, Y: {y}")
...
# Monica extension Land ( Does not work as intended)
    def land(self):
        ...
        self.publish_vehicle_command(VehicleCommand.VEHICLE_CMD_NAV_LAND,
                                     param4=0.0,
                                     param5=44.287249805435074,
                                     param6=-76.4278450235979)
    ...
    def xy_to_gps(self, x, y):
        d_lat = x / EARTH_RADIUS
        d_lon = y / (EARTH_RADIUS * math.cos(self.home_lat * math.pi/180))
    
        lat = self.home_lat + (d_lat * 180/math.pi)
        lon = self.home_lon + (d_lon * 180/math.pi)
    
        return round(lat, 6), round(lon, 6)
    
    def go_through_gps_points(self, points: tuple, radius: float):
        """
        Go through a list of GPS points.
        """
        lat, lon, alt = points[self.waypoint_index]
        x, y = self.gps_to_xy(lat, lon)
        self.publish_waypoint_setpoint(x, y, alt)
        print(f"Gooning it {abs(self.local_position.x - x)}, {abs(self.local_position.y - y)}")
        if abs(self.local_position.x - x) < radius and abs(self.local_position.y - y) < radius:
            print(f"Reached waypoint {self.waypoint_index + 1}")
